  ...
  return new Promise((resolve, reject) => {
    ...
    request(options)
      .then(response => {
        ...
        reject(new Error("Not able to retrieve circles"));
      });
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    request(options)
      .then(response => {
        ...
        reject(new Error("not able to retrive circle"));
      });
  });
}

/**
* Fetch a places by circle ID
*/
module.exports.places = function(session, circleId) {
  return new Promise((resolve, reject) => {
    if (!session) throw new Error("session not specified");
    if (!circleId) throw new Error("circleId not specified");
    const LIFE360_CIRCLE_URL = `https://www.life360.com/v3/circles/${circleId}/places`
    const options = {
      url: LIFE360_CIRCLE_URL,
      headers: {
        'Authorization': `${session.token_type} ${session.access_token}`
    ...
    request(options)
      .then(response => {
        ...
        reject(new Error("Not able to retrieve place"));
      });
  });
}